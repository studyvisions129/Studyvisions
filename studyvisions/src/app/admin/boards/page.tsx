  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-slate-900">Education Boards</h1>
          <p className="text-slate-500 mt-1">Manage top-level academic boards (e.g., CBSE, State Boards).</p>
        </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden flex flex-col relative">
        <div className="p-4 sm:p-6 border-b border-slate-100 flex flex-col sm:flex-row gap-4 justify-between items-center bg-slate-50/50">
          <div className="relative w-full sm:max-w-md group">
            ...
            <form action="/admin/boards" method="GET">
              <input
                type="text"
                name="query"
                defaultValue={query}
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-white border-b border-slate-100 text-xs font-semibold text-slate-400 uppercase tracking-wider">
                <th className="px-6 py-4">Board Info</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {boards.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center justify-center text-slate-400">
                      ...
                      <p className="font-medium text-slate-600">No boards found</p>
                ...
                boards.map((board) => (
                  <tr key={board.id} className="hover:bg-slate-50/80 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="font-bold text-slate-900">{board.name}</div>
                    ...
                    <td className="px-6 py-4 text-sm text-slate-500">
                      <code className="bg-slate-100 px-2 py-1 rounded-md text-slate-700">/{board.slug}</code>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <span className={cn(
                        ...
                        {board.isActive && <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 mr-1.5"></span>}
        ...
        {totalPages > 1 && (
          <div className="p-4 sm:p-6 border-t border-slate-100 flex items-center justify-between bg-slate-50/30">
            ...
            <div className="flex items-center gap-2">
              <Link 
                href={currentPage > 1 ? `/admin/boards?page=${currentPage - 1}${query ? `&query=${query}` : ''}` : '#'}
              ...
              <Link 
                href={currentPage < totalPages ? `/admin/boards?page=${currentPage + 1}${query ? `&query=${query}` : ''}` : '#'}
